...
curl -s http://localhost:5021/swagger/v1/swagger.json > api-spec.json

# Check if the download was successful
if [ ! -s api-spec.json ]; then
...
mkdir -p src/frontend/dbmaker-frontend/src/app/api

# Generate TypeScript client using a simple approach
echo "🔧 Generating TypeScript API client..."

# Create a basic TypeScript client
cat > src/frontend/dbmaker-frontend/src/app/api/api-client.ts << 'EOF'
...
echo "📁 Client saved to: src/frontend/dbmaker-frontend/src/app/api/api-client.ts"
