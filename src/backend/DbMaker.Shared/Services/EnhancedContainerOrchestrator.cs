...
public class EnhancedContainerOrchestrator : IContainerOrchestrator
{
    private DockerClient? _dockerClient;
    private readonly ILogger<EnhancedContainerOrchestrator> _logger;
    ...
    public EnhancedContainerOrchestrator(ILogger<EnhancedContainerOrchestrator> logger)
    {
        _logger = logger;
    ...
    private void LoadExistingPortMappings()
    {
        try
        {
            // Load existing container ports to avoid conflicts
            var existingContainers = System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo
            {
                FileName = "docker",
                Arguments = "ps --filter \"label=dbmaker.userId\" --format \"{{.Labels}}\"",
                RedirectStandardOutput = true,
                UseShellExecute = false,
                CreateNoWindow = true
            });

            existingContainers?.WaitForExit();
            if (existingContainers?.ExitCode == 0)
            {
                var output = existingContainers.StandardOutput.ReadToEnd();
                // Parse container labels to extract ports and mark them as used
                // This is a simplified approach - in production, you'd want to parse the labels properly
                _logger.LogInformation("Loaded existing container mappings");
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to load existing port mappings");
        }
    }
    ...
    {
        // For the enhanced orchestrator, we can add some async port validation if needed
    ...
    private string GenerateConnectionString(DatabaseTemplate template, int port, Dictionary<string, string> environment, string subdomain)
    {
        return template.Type switch
        {
            ...
            "postgresql" => $"postgresql://{environment.GetValueOrDefault("POSTGRES_USER", "admin")}:{environment.GetValueOrDefault("POSTGRES_PASSWORD", "password")}@{subdomain}.mydomain.com:80/{environment.GetValueOrDefault("POSTGRES_DB", "userdb")}",
